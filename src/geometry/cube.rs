...
impl Cube {
    pub fn new(w: i32, h: i32, d: i32) -> Cube {
        ...
        Cube::build(&mut is, &mut vs, &mut ns, 2, 1, 0, -1, -1, d, h, w, ds, hs); // px
        Cube::build(&mut is, &mut vs, &mut ns, 2, 1, 0, 1, -1, d, h, -w, ds, hs); // nx
        Cube::build(&mut is, &mut vs, &mut ns, 0, 2, 1, 1, 1, w, d, h, ws, ds); // py
        Cube::build(&mut is, &mut vs, &mut ns, 0, 2, 1, 1, -1, w, d, -h, ws, ds); // ny
        Cube::build(&mut is, &mut vs, &mut ns, 0, 1, 2, 1, -1, w, h, d, ws, hs); // pz
        Cube::build(&mut is, &mut vs, &mut ns, 0, 1, 2, -1, -1, w, h, -d, ws, hs); // nz
        Cube {
            indices: is,
            vertices: vs,
        }
